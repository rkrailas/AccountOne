...
class BillingNotice extends Component
{
    ...
    public function render()
    {
        ...
        //-----
        $strsql = "SELECT a.billingno, a.billingdate, a.duedate, a.amount, b.customerid || ': ' ||b.name as customername
                ,sum(d.amount-d.paidamount) as balance
            FROM billingnotice a
            JOIN customer b ON a.customerid=b.customerid
            JOIN billingnoticedetail c ON a.billingno=c.billingno
            JOIN taxdata d ON c.taxdataid=d.id
            WHERE a.closed=true AND d.purchase=false AND d.iscancelled=false 
                AND a.billingdate BETWEEN '" . $this->sDate . "' AND '" . $this->eDate . "' AND d.amount <> d.paidamount
                AND (a.billingno ILIKE '%" . $this->searchTerm . "%'
                    OR b.customerid ILIKE '%" . $this->searchTerm . "%'
                    OR b.name ILIKE '%" . $this->searchTerm . "%'
                    OR CAST(a.amount AS TEXT) ILIKE '%" . $this->searchTerm . "%')
            GROUP BY a.billingno, a.billingdate, a.duedate, a.amount, b.customerid, b.name
            ORDER BY " . $this->sortBy . " " . $this->sortDirection;

        $billingNotices = (new Collection(DB::select($strsql)))->paginate($this->numberOfPage);

        return view('livewire.finance.billing-notice',[
