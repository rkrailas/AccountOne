...
class ListGjournal extends Component
{
    ...
    public function render()
    {
        ...
        $strsql = "SELECT account, accnameother AS accname FROM account WHERE detail=true ORDER BY account";
        $this->accountNos = DB::select($strsql);
        ...
        if($this->book){
            ...
            $strsql = "SELECT a.gltran,a.gjournaldt,a.gldescription,b.other
            FROM gltran a
            JOIN misctable b ON a.gjournal=b.code AND b.tabletype='JR'
            WHERE a.gltran <> '' AND a.gjournal = '" . $this->book . "'
                AND (a.gltran ILIKE '%" . $this->searchTerm . "%'
                    OR a.gldescription ILIKE '%" . $this->searchTerm . "%')
            GROUP BY a.gltran,a.gjournaldt,a.gldescription,b.other
            ORDER BY " . $this->sortBy . " " . $this->sortDirection;

            $gltrans = (new Collection(DB::select($strsql)))->paginate($this->numberOfPage);
        ...
        return view('livewire.account.list-gjournal',[
            'gltrans' => $gltrans,
        ]);
    }
}