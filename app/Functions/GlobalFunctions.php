<?php 
    ...
    function getTaxSvNunber($bookid) //Support SO
    {
        $newDocNo = "";
        $data = DB::table('misctable')
                ->select('ram_lasttaxsvnumber', 'ram_prefix_taxsvnumber')
                ->where('tabletype', 'JR')
                ->where('code', $bookid)
                ->get();
        $data2 = explode("-" , $data[0]->ram_lasttaxsvnumber);        

        if (count($data2)){
            if ($data2[0] == $data[0]->ram_prefix_taxsvnumber . date_format(now(),"ym")){

                //Create new number
                $newDocNo = intval($data2[1]) + 1;
                $newDocNo = $data2[0] . "-" . sprintf("%06d", $newDocNo);

                DB::statement("UPDATE misctable SET ram_lasttaxsvnumber=? where tabletype=? and code=?"
                , [$newDocNo,"JR",$bookid]);
            }else{
                $newDocNo = $data[0]->ram_prefix_taxsvnumber . date_format(now(),"ym") . "-000001";
