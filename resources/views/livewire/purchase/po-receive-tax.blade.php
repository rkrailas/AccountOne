<div>
        ...
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    ...
                    <!-- /.ปุ่มซ่อนเมนู -->
                    <h1 class="m-0 text-dark">รับสินค้าพร้อมใบกำกับ</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item">ระบบจัดซื้อ</li>
                        <li class="breadcrumb-item active">รับสินค้าพร้อมใบกำกับ</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                ...
                <div class="col">
                    <table class="table table-md table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td scope="col">{{ $loop->iteration + $purchaseOrders->firstitem()-1  }}</td>
                                <td scope="col">{{ $purchaseOrder->ponumber }} </td>
                                <td scope="col">{{ \Carbon\Carbon::parse($purchaseOrder->podate)->format('Y-m-d') }} </td>
                                <td scope="col">{{ $purchaseOrder->name }} </td>
                                <td scope="col">{{ number_format($purchaseOrder->pototal,2) }} </td>
                                <td scope="col">{{ \Carbon\Carbon::parse($purchaseOrder->transactiondate)->format('Y-m-d') }} </td>
                                <td>
                                    <a href="" wire:click.prevent="edit('{{ $purchaseOrder->ponumber }}')">
                                        <i class="fa fa-edit mr-2"></i>
                                    </a>
                                    <a href="" wire:click.prevent="confirmDelete('{{ $purchaseOrder->ponumber }}')">
    ...
    @include('livewire.purchase._modal-po-receive-tax')
    @include('livewire.sales._modal-list-item')
    @include('livewire._modalGenGL')
    @include('livewire.sales._modal-sn-out')
    @include('livewire.inventory._modalLotNumberOut')
</div>
